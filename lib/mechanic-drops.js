 ...
 * data/item-descriptions.json (see lib/ninja-api.js). The file is keyed by game (both `poe1` and
 * `poe2`). Re-seed it with scripts/discover-mechanic-drops.js at each new league; a missing/absent
 * file (or game key) is a graceful "no mechanics" (empty map), not an error.
 *
...
/** The mechanic map for one game ({} if the file/game key is absent). The top-level `_note`
 * documentation key sits beside the per-game keys, so indexing by game never returns it. */
